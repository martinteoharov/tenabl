...
// token time left
const ttl = (token: { exp?: number }): number => {
  if (!token.exp) return Infinity;
  return token.exp - Date.now() / 1000;
};

const decodeToken = <T extends unknown>(token: string): T & { exp?: number } => {
  try {
 ...
 */
export const authService = (opts: AuthConfig): AuthService => {
  ...
  const createTokenVar = (): Variable<string> => {
